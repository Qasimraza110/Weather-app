...
function App() {
  ...
  >
      <div className="p-8 max-w-lg w-full bg-white bg-opacity-80 rounded-2xl shadow-2xl space-y-8 backdrop-blur-sm">
        <h1 className="text-3xl font-extrabold text-center text-blue-700 mb-4 tracking-wide">
          🌦 Weather App
        </h1>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={getWeather}
            className={`bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold transition ${loading ? "opacity-60 cursor-not-allowed" : ""}`}
            disabled={loading}
          >
        ...
        <div>
          ...
          <ul className="space-y-1">
            {history.map((h) => (
              <li
                key={h._id}
                className="flex items-center justify-between bg-blue-50 rounded px-3 py-1 hover:bg-blue-100 transition"
              >
                <div
                  className="cursor-pointer text-blue-600 hover:underline hover:text-blue-800 transition"
                ...
                <button
                  className="ml-2 text-red-500 hover:text-red-700 transition text-lg"
                  title="Delete"
